...
public class SQLDHSessionStore extends SQLiteOpenHelper implements DHSessionStoreInterface {
	...
	public static final String COLUMN_NEGOTIATED_VERSION = "negotiatedVersion";
	...
	public void onCreate(SQLiteDatabase db) {
		db.execSQL("CREATE TABLE `" + SESSION_TABLE + "` (" +
			...
			"`" + COLUMN_NEGOTIATED_VERSION + "` INTEGER, " +
	...
	public DHSession getBestDHSession(String myIdentity, String peerIdentity) throws DHSessionStoreException {
		...
		try (Cursor cursor = this.getReadableDatabase().query(
			SESSION_TABLE,
			new String[]{COLUMN_SESSION_ID, COLUMN_MY_IDENTITY, COLUMN_PEER_IDENTITY, COLUMN_NEGOTIATED_VERSION, COLUMN_MY_EPHEMERAL_PRIVATE_KEY, COLUMN_MY_EPHEMERAL_PUBLIC_KEY, COLUMN_MY_CURRENT_CHAIN_KEY_2_DH, COLUMN_MY_COUNTER_2_DH, COLUMN_MY_CURRENT_CHAIN_KEY_4_DH, COLUMN_MY_COUNTER_4_DH, COLUMN_PEER_CURRENT_CHAIN_KEY_2_DH, COLUMN_PEER_COUNTER_2_DH, COLUMN_PEER_CURRENT_CHAIN_KEY_4_DH, COLUMN_PEER_COUNTER_4_DH},
			selection,
			new String[] { myIdentity, peerIdentity },
			null,
			null,
	...
	public void storeDHSession(DHSession session) throws DHSessionStoreException {
		...
		cv.put(COLUMN_NEGOTIATED_VERSION, session.getNegotiatedVersion().getNumber());
	...
	private DHSession dhSessionFromCursor(Cursor cursor) throws DHSessionStoreException {
		try {
			return new DHSession(
				new DHSessionId(cursor.getBlob(cursor.getColumnIndexOrThrow(COLUMN_SESSION_ID))),
				cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_MY_IDENTITY)),
				cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_PEER_IDENTITY)),
				Version.forNumber(cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_NEGOTIATED_VERSION))),
