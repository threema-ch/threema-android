...
public abstract class MsgpackBuilder {
    ...
    @NonNull
    final public ByteBuffer consume(MessageBufferPacker packer) {
        if (this.consumed) {
    ...
    @NonNull
    final public ByteBuffer consume(MessagePack.PackerConfig config) {
        return this.consume(config.newBufferPacker());
    }

    @NonNull
    final public ByteBuffer consume() {
