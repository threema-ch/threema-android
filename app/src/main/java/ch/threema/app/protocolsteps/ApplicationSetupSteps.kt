...
fun runApplicationSetupSteps(serviceManager: ServiceManager): Boolean {
    ...
    if (serviceManager.multiDeviceManager.isMdDisabledOrSupportsFs) {
        serviceManager.taskManager.schedule(FSRefreshStepsTask(solicitedContactIdentities))
    }
...
private fun GroupModelOld.toNewGroupModel(groupModelRepository: GroupModelRepository): GroupModel? {
